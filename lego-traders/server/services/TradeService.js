...
class TradeService {
  async tradeResponse(id, status) {
    ...
    if (!trade) {
      throw new BadRequest("Bad Trade ID");
    }
    ...
    if (trade.status == "accepted") {
      ...
      if (!offeredSet) {
        throw new BadRequest("Bad LegoSet ID");
      }
      const requestedSet = await dbContext.LegoSets.findById(
        trade.requestedSetId
      );
      if (!requestedSet) {
        throw new BadRequest("Bad LegoSet ID");
      }
  ...
  async rejectOtherRequested(requestedSetId, id) {
    ...
    if (trade.length) {
      for (let i = 0; i < trade.length; i++) {
        let t = trade[i];
        if (!t) {
          throw new BadRequest("bad trade");
        }
